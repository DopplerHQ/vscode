...
module.exports = async function () {
  ...
  if (!(hasCurl || hasWget) || !hasGPG) {
    ...
    if (response === "Open Documentation") {
      vscode.env.openExternal("https://docs.doppler.com/docs/install-cli");
    }

    throw new Error(
      "Automated install of Doppler CLI failed. Please install manually."
    );
  }

  if (hasCurl) {
    await helpers.terminal.prompt(
  ...
  await helpers.interval(2000, 300000, async function (clearInterval) {
    if (await doppler.auth.hasDopplerCLI()) {
      ...
      vscode.window.showInformationMessage(
        `The Doppler CLI has been installed`
      );
